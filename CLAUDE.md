...
**A new module is registered in one place**: add a `ModuleDescriptor` to `MODULES` in `app/module_registry.py`. `app/all_models.py` (Alembic reads metadata from there, so an unregistered model silently produces an empty migration), `app/main.py` (routers) and `app/core/celery_app.py` (`imports=TASK_MODULES`) all read from that tuple — do not add imports to them directly. The one deliberate exception is the `/mcp` sub-app mounted at the end of `main.py`: `routers()` accepts only an `APIRouter`, and the whole MCP surface is a single `POST /mcp`, which the path-prefix write gate cannot judge per tool — `copilot.mcp._authorize_write` rebuilds that check where the execution mode is known. The second, separate edit is the flow matrix: every new table needs a row. The descriptor's `permission_actions` and `metric_prefixes` have no reader; they are a declared inventory, while the enforced permission is the `x-permission` on the route.
