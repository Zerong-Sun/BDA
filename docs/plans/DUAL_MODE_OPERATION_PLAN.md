> **规划资料，非当前功能清单。** 本文保留阶段目标和历史核验；当前行为见 [Autopilot 边界](../AUTOPILOT_CAMPAIGNS.md)与 [Copilot 服务](../COPILOT_SERVICE_GUIDE.md)。未完成部分继续保留，不因本次整理视为完成或废弃。
...
**计算那一环已对真实集群验证**（2026-09-06，所有者完成登录后）。
`backend_v2/scripts/check_lsf_roundtrip.py` 把 `LSFAdapter` 的三个动作在 Qiming 上跑通：
提交（`4250455`/`4250457`，`v3-64`，`Done successfully`，CPU 0.1 秒）、
重复提交返回同一作业号、`collect` 解析作业自己写的 manifest。
作业刻意做到最小：一个 slot、几秒钟、`hostname` 加一个标记串 —— 这是机制测试，不是工作负载。
`-n 1` / `ptile=1` / `BDA_CPUS` 同源，并且先查过队列没有合并 `GPU_REQ`
（这台集群上有些队列会给没申请 GPU 的作业也挂一块独占 GPU）。

**仍未达成**：**整条链在一次真实运行里连起来**。每一环都有测试，计算环现在对真实集群也有了，
但 campaign → adapter → submission → outbox → publisher → dispatch → collect → 制品/候选 →
下一阶段，从来没有在一次真实运行里依次走完过。

差的不是代码，是一个会产出真实文件的已注册插件 —— 也就是一次真正的科学计算作业。
注册表里现在只有 AlphaFold、RFdiffusion 这类真实模型，**没有可以拿来凑数的空转插件**。
为了勾掉一个文档条目去跑一次真实科学作业，花的是所有者的机时，那是所有者的决定，不是我的。
