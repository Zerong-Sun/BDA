...
try {
  ...
  await page.locator('.bot-roster-item', { hasText: 'Structuralist' }).click()
  await page.getByRole('heading', { level: 1, name: 'Structuralist', exact: true }).waitFor()
  assert.ok(page.url().includes('/bots/structuralist?project=proj_browser'))
  assert.equal(await page.locator('.bot-roster-item', { hasText: 'Structuralist' }).getAttribute('aria-current'), 'page')
  await page.getByRole('tab', { name: 'Conversation', exact: true }).click()
  await page.locator('.bot-chat-surface').waitFor()
  await screenshot(page, 'bot-structuralist-en-light')
  ...
  await detail.getByRole('heading', { level: 1, name: 'Librarian', exact: true }).waitFor()
  ...
  await screenshot(detail, 'bot-librarian-en-light')
  ...
  assert.ok(detail.url().includes('/bots/librarian?') && detail.url().includes('run=task-browser'))
  await detail.goBack()
  const writesBeforeAssign = writes.length
  await detail.getByRole('button', { name: 'Assign a task to Librarian', exact: true }).click()
  await detail.getByLabel('Task goal', { exact: true }).waitFor()
  assert.ok(detail.url().includes('/bots?project=proj_browser&view=tasks'))
  assert.equal(await detail.getByRole('group', { name: 'Task owner', exact: true }).getByRole('button', { name: /^Librarian/ }).getAttribute('aria-pressed'), 'true')
  ...
  checks.push('Each Bot page holds its tasks at durable URLs, links its workbenches, and assigning only prepares the composer')
