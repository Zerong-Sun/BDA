...
const bot = (id: string, title: string, stance: string, extra: Record<string, unknown> = {}) => ({ id, title, title_zh: title, phase: 0, stance, summary: `${title} summary`, charter: `${title} charter: never invent evidence.`, capabilities: [], handoff: [], reviews: [], directs: [], reviewed_by: [], triggers: [], task_service: null, task_write_tools: [], ...extra })
const roster = [
  bot('conductor', 'Conductor', 'direct', { phase: -1, directs: ['librarian', 'planner'] }),
  bot('librarian', 'Librarian', 'produce', { phase: 1, handoff: ['planner'], reviewed_by: ['auditor'], task_service: 'literature', task_write_tools: ['start_literature_search'] }),
  bot('planner', 'Planner', 'produce', { phase: 4, task_service: 'planning' }),
  bot('auditor', 'Auditor', 'review', { phase: 9, reviews: ['librarian'] }),
]
...
const runs = [run('task-1', 'Search PD1 literature', 'librarian'), run('task-2', 'Plan the route', 'planner'), run('task-3', 'Delegated search', 'librarian', { parent_run_id: 'task-0' })]
...
beforeEach(() => {
  ...
  server.use(
    ...
    http.get('/api/v2/copilot/projects/:projectId/handoffs', () => HttpResponse.json({ items: [
      handoff('h1', 'librarian', 'planner', 'Evidence is ready for routing'),
...
describe('Bot responsibility page', () => {
  it('shows what an operator owns, holds and hands over, and scopes the conversation to it', async () => {
    renderAt('/bots/librarian?project=project-bot')
    expect(await screen.findByRole('heading', { level: 1, name: 'Librarian' })).toBeInTheDocument()
    expect(screen.getByText('Librarian charter: never invent evidence.')).toBeInTheDocument()
    ...
    expect(useAppStore.getState().copilotSessions['project-bot']?.bot).toBe('librarian')
  })

  it('assigns a task by preparing the composer, never by starting one', async () => {
    renderAt('/bots/librarian?project=project-bot')
    fireEvent.click(await screen.findByRole('button', { name: 'Assign a task to Librarian' }))
    ...
    expect(useAppStore.getState().copilotTaskDrafts['project-bot']).toMatchObject({ bot: 'librarian', preview: true, writes: [] })
    expect(posts).toBe(0)
  })
  ...
  it('offers a conversation instead of assignment to an operator that owns no task', async () => {
    ...
    expect(within(screen.getByRole('navigation', { name: 'Works with' })).getByRole('link', { name: /Reviews.*Librarian/ })).toBeInTheDocument()
  ...
  it('keeps assignment unavailable to a read-only viewer', async () => {
    ...
    renderAt('/bots/librarian?project=project-bot')
    expect(await screen.findByRole('button', { name: 'Assign a task to Librarian' })).toBeDisabled()
  })
  ...
  it('opens a held task at a durable URL on the operator page', async () => {
    ...
    expect(window.location.hash).toBe('#/bots/librarian?project=project-bot&run=task-1')
  })
