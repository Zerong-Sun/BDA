...
import { ArrowDownIcon, ArrowUpIcon, CaretUpDownIcon, CheckIcon, ArrowLineLeftIcon, ArrowLineRightIcon, ArrowLeftIcon, ArrowRightIcon, SlidersHorizontalIcon, PushPinSlashIcon } from "@phosphor-icons/react"
...
function DataGridColumnHeaderInner<TData, TValue>({
  ...
  const headerButtonClassName = cn(
    "text-secondary-foreground/80 hover:bg-secondary data-[state=open]:bg-secondary hover:text-foreground data-[state=open]:text-foreground px-2 font-normal h-6 rounded-none",
    className
  )
  ...
  const sortIcon =
    ...
    (isSorted === "desc" ? (
      ...
      <CaretUpDownIcon className="mt-px size-3.25" aria-hidden="true" />
    ))
  ...
  const menuItems = useMemo(() => {
    ...
    if (props.tableLayout?.columnsPinnable && canPin) {
      ...
      items.push(
        ...
        >
          <ArrowLineLeftIcon className="size-3.5!" aria-hidden="true" />
        ...
        >
          <ArrowLineRightIcon className="size-3.5!" aria-hidden="true" />
      ...
      <div className="-ms-2 flex h-full items-center justify-between gap-1.5">
        ...
        {props.tableLayout?.columnsPinnable && canPin && isPinned && (
          <Button
            size="icon-sm"
            variant="ghost"
            className="rounded-none -me-1 size-7"
            onClick={() => column.pin(false)}
            aria-label={`Unpin ${resolvedTitle} column`}
            title={`Unpin ${resolvedTitle} column`}
          >
            <PushPinSlashIcon className="size-3.5! opacity-50!" aria-hidden="true" />
          </Button>
        )}
      </div>
    )
