  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="bg-primary rounded-b-none absolute top-0 left-0 -translate-x-1/2 shadow-xs"
        style={{ width: 5 }}
      />
    </div>
  )
...
function DataGridTableBodyRowSkeletonCell<TData>({
  ...
  return (
    ...
    >
      {children ?? <Skeleton className="h-4 w-full" />}
    </td>
  )
}
...
function DataGridTableLoader() {
  ...
  return (
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
      <div className="text-muted-foreground bg-card rounded-none flex items-center gap-2 border px-4 py-2 text-sm leading-none font-medium">
...
function DataGridTableRowPin<TData>({ row }: { row: Row<TData> }) {
  ...
  return (
    <button
      ...
      className={cn(
        "text-muted-foreground hover:text-foreground rounded-none inline-flex size-7 items-center justify-center transition-colors",
