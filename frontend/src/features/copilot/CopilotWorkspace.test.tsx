...
const bot = (id: string, title: string, stance: string, extra: Record<string, unknown> = {}) => ({ id, title, title_zh: title, phase: 0, stance, summary: `${title} summary`, charter: '', capabilities: [], handoff: [], reviews: [], directs: [], reviewed_by: [], triggers: [], task_service: null, task_write_tools: [], ...extra })
// The literature recipe offers note authoring, but its owner does not hold it.
const roster = [
  bot('librarian', 'Librarian', 'produce', { phase: 1, reviewed_by: ['auditor'], triggers: ['literature'], task_service: 'literature', task_write_tools: ['start_literature_search'] }),
  bot('planner', 'Planner', 'produce', { phase: 4, triggers: ['route'], task_service: 'planning' }),
...
describe('task-centered Copilot', () => {
  ...
  it('retains a reviewed task draft on remount without starting a task', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: /^Librarian/ }))
  ...
  it('recovers the unavailable service list without losing the request', async () => {
    ...
    expect(await screen.findByRole('button', { name: /Librarian.*Research the evidence/ })).toBeEnabled()
  ...
  it('explains an unavailable owner roster once and recovers it without losing the goal', async () => {
    ...
    expect(await screen.findByRole('button', { name: /^Librarian/ })).toBeEnabled()
    expect(screen.getByLabelText('Task goal')).toHaveValue('Research PD1')
  })
  it('shows a reviewable task scope and sends only explicitly checked writes', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: /^Librarian/ }))
    expect(screen.getByRole('heading', { name: 'Librarian owns: Research the evidence' })).toBeInTheDocument()
    ...
    expect(bodies[0]).toMatchObject({ bot: 'librarian', service_kind: 'literature', authorized_writes: ['start_literature_search'] })
  ...
  it('resolves a page request for a kind of work to the operator that owns it', async () => {
    renderWithProviders(<CopilotWorkspace initialGoal="Review sources" initialService="literature" />)
    expect(await screen.findByRole('heading', { name: 'Librarian owns: Research the evidence' })).toBeInTheDocument()
  })
  it('groups tasks under the operator that owns them', async () => {
    ...
    renderWithProviders(<CopilotWorkspace />)
    const librarian = await screen.findByRole('group', { name: 'Librarian' })
    ...
    expect(groups.indexOf('Librarian')).toBeLessThan(groups.indexOf('No assigned owner'))
  })
