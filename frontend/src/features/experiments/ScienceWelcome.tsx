...
export function ScienceWelcome({ onCreate }: { onCreate: () => void }) {
  ...
  return <section className="science-welcome">
    <div>
      ...
      <h1>{zh ? <>好问题，<br />让研究向前。</> : <>Good questions.<br />Meaningful progress.</>}</h1>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <Button type="button" variant="outline" className="science-primary" onClick={onCreate}><PlusIcon />{zh ? '新建项目' : 'New project'}</Button>
      </div>
    </div>
    <div className="science-team-preview">
      ...
      <div className="science-preview-footer"><span>{zh ? '当前项目' : 'PROJECT CONTEXT'}</span><span>{activeProject?.name || (zh ? '选择一个项目开始' : 'Choose a project to begin')}</span></div>
    </div>
  </section>
}
