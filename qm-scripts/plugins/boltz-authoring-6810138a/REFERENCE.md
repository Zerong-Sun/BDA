# Boltz-authoring-6810138a — 功能、参数与使用手册
...
本页是独立 authoring 草稿；共用基础工具解释，但以下版本参数/命令摘要来自草稿本身，不能继承基础版本的运行证明。
...
|---|---|---|---|---|
| 2.x-draft.1 | true | unknown | unproven | false |
...
[
  {
    ...
    "required": true,
    "multiple": true,
    "description": "Reviewed receptor–binder complex YAML files; raw FASTA is not accepted."
  }
]
```
...
命令摘要 SHA-256：`ecad28d5818e237d530f8ca65436a40f8eede3a489f383bcebfaf08d6722d334`。

命令文本中出现的 flags（文本清单，不保证所有条件分支执行）：`["--diffusion_samples", "--msa_server_url", "--out_dir", "--recycling_steps", "-maxdepth", "-name", "-o", "-q", "-type", "-z"]`。
