# method_expression_risk — 功能、参数与使用手册

> 文档快照：2026-09-15；由 build_reference_docs.py 生成。配置/文档覆盖不等于运行验证。

表达风险排序：当前仅有 demo-1.0 注册记录，输入/输出/参数 schema 均为空。

[统一规范](../../../docs/plugins/STANDARD.md) · [全部插件总览](../../../docs/plugins/INDEX.md) · [结构化解释源](../../../docs/plugins/profiles/method-expression-risk.json)
...
限制：缺少预测模型/权重、表达宿主适用域、训练验证证据和缺失值策略；不能按名称判断 E. coli 表达。
...
- 缺少预测模型/权重、表达宿主适用域、训练验证证据和缺失值策略；不能按名称判断 E. coli 表达。
